...
The biomedical domain is among the early successes of the semantic web, due to the rapidity with which the biomedical community has made its data available in RDF triple stores \cite{Yip}. Although a plethora of useful biomedical data is currently available in RDF, there is a need for easy-to use systems that do not require the end user to have knowledge of the underlying  structure of the data, and also allow users to run federated queries on multiple SPARQL endpoints.  
...
Many major biological and biomedical data resources, including Gene Ontology, DailyMed, LinkedCT, and Diseasome, are currently available as RDF.   Integrating biomedical data across multiple data sources and automatically extracting specific  knowledge from web resources are crucial tasks for physicians and biologists. These semantic web resources represent valuable repositories of information that can be automatically mined for applications that require biological knowledge.  
...
QueryMed allows users to query many biomedical data sources from a single interface.  The data sources that the user wishes to query are not fixed, and can be extended very easily.  The system was designed with non-expert users of SPARQL in mind, and presents a very intuitive user interface that anybody with basic biomedical domain knowledge can use. 

The system functionality in a nutshell is as follows: A user submits a query from the Query Interface to the SPARQL endpoints via the proxy server.  The query is translated into a SPARQL query for each individual endpoint.  Once the results are obtained from the SPARQL endpoints, they are presented to the user via the Result Browser.  The user can then choose to refine the query by changing some of the original query terms in the Dynamic Facet Creator/Filter, and this process repeats.
...
A more detailed illustration of the QueryMed system highlighting all the components necessary for system functionality is given in Figure \ref{fig:arch_details}.  The system relies on two external resources: 
...
QueryMed is implemented in Java in the backend and JavaScript, HTML and CSS in the frontend.  The JQuery library \cite{Resig} was used to develop an attractive user interface.  In the backend, the Jena library \cite{McBride} is used to run the SPARQL queries. 
