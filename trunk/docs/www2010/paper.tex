...
Publicly available data in the biomedical domain has grown significantly over the recent years.   For example, this data include data on drug discovery~\cite{Sharp, Goble}, clinical trial information that are publicly available at ClinicalTrails.gov, disease genes, phenotypes published by the Online Mendelian Inheritance in Man (OMIM), and etc.  With the linked open data movement, the semantic web community has been very proactive in converting these datasets to RDF~\cite{LinkingData}.  In fact, the biomedical domain is among the early successes of adopting semantic web due to the rapidity with which the community has made its data available in RDF triple stores \cite{Yip}.  
...
Although a plethora of useful biomedical data is currently available in RDF, there is a need for easy-to use systems that do not require the end user to have knowledge of the underlying  structure of the data, and that also allow users to run federated queries on multiple SPARQL endpoints.  Many currently available systems are linked data data browsers such as the Tabulator~\cite{Tabulator}, which allow a user to navigate the data in an exploratory manner, but lack support for efficiently filtering and querying the data. Therefore, there is also a  need for efficient hybrid interfaces that allow both browsing and querying~\cite{Jentzsch}. 
...
QueryMed allows users to query many biomedical data sources from a single interface.  Although there is a default list of sources that the user can choose to query from, this list is  not fixed, and can be extended very easily.  The system was designed with non-expert users of SPARQL in mind, and presents a very intuitive user interface that anybody with basic biomedical domain knowledge can use. 

The system functionality in a nutshell is as follows: A user submits a query from the Query Interface to the SPARQL endpoints via the proxy server.  This is translated into a SPARQL query for each individual endpoint.  Once the results are obtained from the SPARQL endpoints, they are presented to the user via the Result Browser.  The user can then choose to refine the query by changing some of the original query terms in the Dynamic Facet Creator/Filter. This is an iterative process, where the user can refine the query by adding new sources, and tweaking values given to the properties.

In the following sections we explain the system functionality by first giving an overview of the QueryMed system architecture and the design decisions behind some of the crucial components of the system.
...
A general overview of QueryMed architecture is shown in Figure \ref{fig:arch_overview}. The main components of the system are the user interface and the proxy server that retrieves biomedical data from remote SPARQL endpoints. A more detailed illustration of the QueryMed system highlighting all the components necessary for system functionality is given in Figure \ref{fig:arch_details}.  
...
The user interface is one of the primary components of the QueryMed system. It comprises of the following:

\begin{itemize}

\item \textbf{Basic Query Interface:}  In order to make QueryMed easy to use, but still provide a flexible tool that is able to perform a range of queries, we provide an uncluttered interface with only a  blank search box and two buttons as shown in Figure \ref{fig:initial_ui}. The ``Query All" button will perform a keyword-based query over pre-selected data sources. The advanced search option ``Refine Query" allows the user to view additional query options that are handled in the Advanced Query Interface.  

 For example, a physician interested in finding semantic web resources related to coronary artery disease could use “coronary artery disease”  as a search keyword in the input search box and select the "Query All" option.   The simplicity of the basic query interface allows this end user to rapidly and easily browse semantic web resources of interest. 
...
the route of administration of the drug that she is looking for is injection, so she filters the drug query results on the route of administration field using the query term “injection.”   The additional filtering capabilities of the Result Browser allows her  to either browse a large number of query results, or refine her search to view the most relevant search terms.

\item \textbf{Advanced Query Interface:} This allows the user to add new data sources, and give values for properties found at a particular selected data source. Therefore, the user is able to extend the query from a basic keyword search over all the properties as with the basic query interface, to a more targeted query. 

When the ``Refine Query" in the Basic Query Interface is invoked, the user is provided with the default list of data sources in the system as shown in Figure \ref{fig:select}.  This feature allows the user to perform their query only over relevant or trusted data sources.  If none of the desired data sources are available in this list, the user can dynamically add additional data sources as shown in Figure \ref{fig:add_source}.  The ability to dynamically add additional data sources increases the flexibility of  the QueryMed system, allowing users to query endpoints of interest that are not included in the default list.
...
When a data source is selected, the properties list is automatically populated with all the distinct properties available at the selected endpoint. If the user wants to restrict her query based on one or more properties available, she can specify those. This allows users to improve their queries using the underlying structure of the data without having prior knowledge of this structure to obtain more relevant query results. 
...
For example, the physician might be interested in finding relevant clinical trials for 
her patient. However, the clinical trial database LinkedCt is 
not in the default set of endpoints, so she selects the “Refine 
Query” option to choose additional endpoints to search. She 
sees a list of default endpoints, and selects the “Add” option 
to include an additional endpoint. After entering the name 
and URL of the LinkedCT endpoint, she is able to search 
for clinical trials for which her patient may be eligible. 



\item \textbf{Dynamic Facet Creator/Filter:} This allows the user to select a  set of  data sources, load the properties available at these sources, and dynamically construct a complex SPARQL query connected by logical operators to take advantage of the structure of the data at each endpoint as shown in Figure  \ref{fig:facet_creator}. The interface allows displaying of many property lists from different sources simultaneously (grouped by source). Therefore, only information relevant to the endpoint the user is currently examining will be visible at any given time.
...
diseases whose class is “Cardiovascular” or for which the as- 
sociated gene is “ABCA1.” Using the QueryMed advanced 
...
The functionality of the proxy server is to act as an intermediary between the user interface and the remote SPARQL endpoints.  Its functionality is twofold: 
...
\item Cache the results for subsequent queries for refinements to the same query to reduce the network and query execution latency.
\end{enumerate}
...
\item \textbf{Translator:} The Translator is responsible for translating the user query into valid SPARQL syntax.  Translator obtains the parameters to construct the query from the input the user specifies in the Query Interface, and plugs them in to a relevant template. It specifically relies on two services:

\begin{enumerate}

\item \textbf{Run Query:} The purpose of this service is to execute SPARQL queries generated by the Translator, and return  the query results as a JSON object given in Figure \ref{fig:JSON_runquery}.
...
\item \textbf{Advanced Queries:} This service takes as input list of sources, properties, query terms, and logical operators, which are passed to the proxy server as a JSON object given in Figure \ref{fig:JSON_advquery}. 
...
The main contributions of our system are: dynamic construction of complex SPARQL queries based on intuitive user input; dynamic addition of user-specified endpoints; and ability to run queries over multiple endpoints. Because of the unique features of our system, we believe it will be of use to the biomedical community.  We also believe tools such as QueryMed will entice more people to expose their data as linked open data.
