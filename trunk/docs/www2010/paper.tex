...
Although a plethora of useful biomedical data is currently available in RDF, there is a need for easy-to use systems that do not require the end user to have knowledge of the underlying  structure of the data, and also allow users to run federated queries on multiple SPARQL endpoints.  The state of the art seem to be browsing the data using a linked data browser such as the Tabulator~\cite{Tabulator}.  The linked data approach allows a user to navigate the data in an exploratory manner, but lacks support for efficient filtering and querying of the data in a manner that is amenable for domain experts. Therefore, there is a need to have efficient hybrid interfaces that combine both browsing and querying~\cite{Jentzsch}.
